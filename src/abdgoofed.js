...
// AI.JS
// THIS HAS PAIR COMMAND(BUTTONED)
...
king({ nomCom: "gpt", reaction: "📡", categorie: "AI" }, async (dest, zk, commandeOptions) => {
  ...
  try {
    ...
    const response = await fetch(`https://www.noobs-api.000.pe/dipto/gemini?prompt=${question}`);
    const data = await response.json();
...
adams({ nomCom: "gpt4", reaction: "📡", categorie: "AI" }, async (dest, zk, commandeOptions) => {
  const { repondre, arg, ms } = commandeOptions;
...
adams({ nomCom: "bard", aliases: ["gptbard"], categorie: "AI" }, async (dest, zk, commandeOptions) => {
